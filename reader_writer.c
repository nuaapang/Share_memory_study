...
struct semid_ds *buf;/**buffer for IPC_STAR, IPC_SET*/
...
void reader (void){
	...
	/*信号量SN_READ初始化为1（锁定），SN_WRITE初始化为0（未锁定）*/
	sunion.val = 1;
	...
	/*将该共享内存标志为已经销毁的，这样在使用完毕后，将被自动的销毁*/
	shmctl (shmid, IPC_RMID, NULL);
	
	/*将该信号量的标识符写入共享内存，以通知其他的进程*/
	*(int*) shmdata = semid;
	...
	************************/
	while(1){
		...
		locksem(semid, SN_WRITE);/*写上锁，只读取信息*/
		printf("finish \n");
...
void writer(int shmid){
	...
	semid = *(int*) shmdata;
	buffer = shmdata + sizeof(int);
	printf("\n writer begin to run, the id of share memory is %d..\n,the semaphore id is %d..\n",shmid, semid);
...
void delete(void){
	...
	/*****删除信号量*******************/
	if (mysemctl(Semid,0, IPC_RMID, unused)==-1){
		printf("Error releaseing semaphore.\n");
	}
}
void sigdelete(int signum){
	/*****Call exit will conveniently trigger the normal delete item*******************/
	exit(0);
}
void locksem(int semid, int semnum){
	struct sembuf sb;
